...
export async function getAffiliateReferralAnalytics(pool: Pool, req: Request, res: Response) {
  try {
      ...
      partners.map(async (partner) => {
        ...
        referrals.forEach(referral => {
          if (referral.order_items && Array.isArray(referral.order_items)) {
            referral.order_items.forEach((item: any) => {
              allProducts.push({
    ...
    sendError(res, 500, 'Failed to fetch affiliate referral analytics', err)
  }
}
