...
export function registerExtendedRoutes(app: express.Express, pool: Pool, io: SocketIOServer) {
  ...
  app.get('/api/cashback/balance', authenticateToken, async (req: any, res) => {
    try {
  ...
  app.get('/api/nefol-coins', authenticateToken, async (req: any, res) => {
    try {
      const userId = req.userId
      const { rows } = await pool.query(`
  ...
  app.get('/api/coin-transactions', authenticateToken, async (req: any, res) => {
    try {
      const userId = req.userId
  ...
  app.post('/api/cashback/process-retroactive', authenticateToken, async (req: any, res) => {
    try {
      const userId = req.userId
      ...
      sendError(res, 500, 'Failed to process retroactive cashback', err)
    }
  })
  ...
  app.put('/api/user/orders/:order_number/payment-cancelled', authenticateToken, async (req: any, res) => {
    try {
      const userId = req.userId
      ...
      const orderResult = await pool.query(
        ...
        [order_number, userEmail]
      )
      
      if (orderResult.rows.length === 0) {
        return sendError(res, 404, 'Order not found or you do not have permission to update this order')
      }
      ...
      if (order.status === 'confirmed' || order.status === 'delivered' || order.status === 'completed') {
        return sendError(res, 400, 'Cannot update payment status for confirmed orders')
      }
      ...
      try {
        ...
        await pool.query(
          `INSERT INTO order_status_history (order_id, old_status, new_status, note)
           VALUES ($1, $2, $3, $4)`,
          [order.id, order.status, 'pending_payment', 'Payment cancelled by user']
        )
      } catch (e) {
        console.error('Failed to write order status history:', e)
      }
      
      sendSuccess(res, { message: 'Order status updated to pending payment', order_number })
    } catch (err) {
      console.error('Error updating order payment status:', err)
      sendError(res, 500, 'Failed to update order payment status', err)
    }
  })

  app.get('/api/coin-withdrawals', authenticateToken, async (req: any, res) => {
    try {
      const userId = req.userId
      const { rows } = await pool.query(`
        SELECT 
  ...
  app.post('/api/coin-withdrawals', authenticateToken, async (req: any, res) => {
    try {
      ...
      if (amount < 10) {
        return sendError(res, 400, 'Minimum withdrawal is 10 coins')
      }
      
      if (availableCoins < amount) {
        return sendError(res, 400, `Insufficient coins. You have ${Math.floor(availableCoins)} withdrawable coins available. Referral coins can be withdrawn 8 days after the referral purchase.`)
      }
  ...
  app.put('/api/admin/coin-withdrawals/:id/process', async (req, res) => {
    try {
      ...
      if ((status === 'rejected' || status === 'failed') && !rejection_reason) {
        return sendError(res, 400, 'Rejection reason is required when status is rejected or failed')
      }
      ...
      if (status === 'completed' && !withdrawal.razorpay_payout_id) {
        try {
          ...
          return sendError(res, 500, `Failed to create Razorpay payout: ${errorMessage}`, payoutError)
        }
      }
      ...
      if (razorpayPayoutId) { updateFields.push(`razorpay_payout_id = $${values.length + 1}`); values.push(razorpayPayoutId) }
  ...
  app.put('/api/invoice-settings/all', async (req, res) => {
    try {
      ...
      if (companyDetails) await pool.query(`
        INSERT INTO store_settings (setting_key, setting_value)
        VALUES ($1, $2::jsonb)
        ON CONFLICT (setting_key) 
        DO UPDATE SET setting_value = $2::jsonb, updated_at = NOW()
      ...
      if (logoUrl !== undefined) await pool.query(`
        INSERT INTO store_settings (setting_key, setting_value)
        VALUES ($1, $2::text)
        ON CONFLICT (setting_key) 
        DO UPDATE SET setting_value = $2, updated_at = NOW()
      `, ['invoice_logo_url', logoUrl || null])
      if (signatoryPhotoUrl !== undefined) await pool.query(`
        INSERT INTO store_settings (setting_key, setting_value)
        VALUES ($1, $2::text)
        ON CONFLICT (setting_key) 
        DO UPDATE SET setting_value = $2, updated_at = NOW()
  ...
  app.get('/api/invoices/:id/download', async (req, res) => {
    try {
      ...
      const defaultCompanyDetails: any = { companyName: 'Nefol', companyAddress: '', companyPhone: '7355384939', companyEmail: 'info@nefol.com', gstNumber: '', panNumber: '', bankName: '', accountNumber: '', ifscCode: '' }
      const defaultColors: any = { primaryStart: '#667eea', primaryEnd: '#764ba2', accentStart: '#667eea', accentEnd: '#764ba2' }
      ...
      const invoiceHtml = generateAmazonInvoiceHTML(order, companyDetails, taxSettings, terms, signature, currency, logoUrl, signatoryPhotoUrl, shipmentInfo)
  ...
  function generateInvoiceHTML(order: any, companyDetails: any, colors: any, taxSettings: any, terms: string, signature: string, currency: string, logoUrl: string | null = null, signatoryPhotoUrl: string | null = null): string {
    ...
    try {
      ...
      const formatShippingAddress = () => {
        if (!order.shipping_address) return 'N/A'
      ...
      const formatBillingAddress = () => {
        ...
        if (!billingAddr) return 'N/A'
        if (typeof billingAddr === 'string') return billingAddr
