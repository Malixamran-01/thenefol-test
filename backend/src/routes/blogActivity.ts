...
router.get('/authors/:authorId/activity', async (req, res) => {
  try {
    ...
    // Profile activity = this author's own actions only.
    // Incoming likes/comments/reposts on their posts live in blog_notifications (Nefol Social → Activity tab).
    ...
    // Combine own activities and sort by date
    const allActivities = [
      ...likedPosts,
      ...commentedPosts,
      ...publishedPosts,
    ...
    console.log(`[activity] profile timeline: ${allActivities.length} (liked=${likedPosts.length} commented=${commentedPosts.length} published=${publishedPosts.length} reposted=${repostedPosts.length})`)
