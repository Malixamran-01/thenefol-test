  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Filter combos related to hair
        const hairCombos = data.filter((product: any) => {
          const category = (product.category || '').toLowerCase()
          ...
          // Check if it's hair-related
          return title.includes('hair') || 
                 productType === 'hair' ||
                 title.includes('hair care')
        })
        setCombos(hairCombos)
      }
    } catch (error) {
        ...
        {combos.length > 0 && (
          <div className="mb-16">
            <div className="text-center mb-12">
              ...
              <p className="text-sm sm:text-base font-light max-w-2xl mx-auto tracking-wide" style={{ color: '#666', letterSpacing: '0.05em' }}>
                Complete hair care solutions in curated combo packs. Save more with our specially designed hair care bundles.
              </p>
            </div>
                            ...
                            if (addItem) {
                              try {
                                addItem({
                                  ...
                                  description: combo.description || 'Premium hair care combo pack'
                                })
                              } catch (error) {
                                console.log('Authentication required for cart operation')
                              }
