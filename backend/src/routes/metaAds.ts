...
export async function saveAdsConfig(pool: Pool, req: Request, res: Response) {
  try {
    ...
    sendError(res, 500, 'Failed to save Meta Ads config', err)
  }
}

export async function getAdsConfig(pool: Pool, req: Request, res: Response) {
  try {
    await ensureTables(pool)
    const { rows } = await pool.query(
...
export async function getCampaigns(pool: Pool, req: Request, res: Response) {
  try {
    await ensureTables(pool)
    const { rows } = await pool.query(`
    ...
    sendError(res, 500, 'Failed to get campaigns', err)
  }
}

export async function createCampaign(pool: Pool, req: Request, res: Response) {
  try {
    await ensureTables(pool)
    ...
    if (!accessToken || !adAccountId) {
      return sendError(res, 400, 'Meta Ads not configured. Please set up access token and ad account ID.')
    }
    ...
    if (!name || !objective) {
      return sendError(res, 400, 'Campaign name and objective are required')
    }
    ...
    if (result.error) {
      return sendError(res, 400, result.error.message || 'Failed to create campaign')
    }

    const campaignId = result.id

    // Fetch full campaign details
    const campaignDetails = await callMetaAPI(
      campaignId,
      'GET',
    ...
    sendSuccess(res, { campaign_id: campaignId, ...campaignDetails }, 201)
  } catch (err: any) {
    sendError(res, 500, 'Failed to create campaign', err)
  }
}

export async function updateCampaign(pool: Pool, req: Request, res: Response) {
  try {
    ...
    if (result.error) {
      return sendError(res, 400, result.error.message || 'Failed to update campaign')
    }
...
export async function deleteCampaign(pool: Pool, req: Request, res: Response) {
  try {
    const { id } = req.params as any
    const accessToken = await getMetaAccessToken(pool)
    if (!accessToken) {
    ...
    if (result.error && !result.success) {
      return sendError(res, 400, result.error.message || 'Failed to delete campaign')
    }
    ...
    sendError(res, 500, 'Failed to delete campaign', err)
  }
}

// ==================== AD SETS ====================

export async function getAdSets(pool: Pool, req: Request, res: Response) {
  try {
    await ensureTables(pool)
    ...
    sendError(res, 500, 'Failed to get ad sets', err)
  }
}

export async function createAdSet(pool: Pool, req: Request, res: Response) {
  try {
    await ensureTables(pool)
    const accessToken = await getMetaAccessToken(pool)
    const adAccountId = await getAdAccountId(pool)
    ...
    if (!accessToken || !adAccountId) {
      return sendError(res, 400, 'Meta Ads not configured')
    }
    ...
    if (!campaign_id || !name) {
      return sendError(res, 400, 'Campaign ID and ad set name are required')
    }

    // Build comprehensive targeting object
    const targetingObject: any = {
    ...
    if (targeting?.custom_audiences && targeting.custom_audiences.length > 0) {
      targetingObject.custom_audiences = targeting.custom_audiences.map((id: string) => ({ id }))
    }
    ...
    if (targeting?.lookalike_audiences && targeting.lookalike_audiences.length > 0) {
      targetingObject.flexible_spec = [
        {
          lookalike_specs: targeting.lookalike_audiences.map((aud: any) => ({
            lookalike_spec: {
    ...
    if (result.error) {
      return sendError(res, 400, result.error.message || 'Failed to create ad set', result.error)
    }

    const adsetId = result.id

    // Fetch full ad set details
    const adSetDetails = await callMetaAPI(
      adsetId,
      'GET',
      { fields: 'id,name,status,daily_budget,lifetime_budget,billing_event,optimization_goal,targeting,start_time,end_time,created_time,updated_time,bid_strategy' },
      accessToken
    )
    ...
    sendSuccess(res, { adset_id: adsetId, ...adSetDetails }, 201)
  } catch (err: any) {
    console.error('Ad set creation error:', err)
    sendError(res, 500, 'Failed to create ad set', err)
  }
}

export async function updateAdSet(pool: Pool, req: Request, res: Response) {
  try {
    const { id } = req.params as any
    const accessToken = await getMetaAccessToken(pool)
    if (!accessToken) {
    ...
    if (result.error) {
      return sendError(res, 400, result.error.message || 'Failed to update ad set')
    }
    ...
    sendError(res, 500, 'Failed to update ad set', err)
  }
}

export async function deleteAdSet(pool: Pool, req: Request, res: Response) {
  try {
    const { id } = req.params as any
    const accessToken = await getMetaAccessToken(pool)
    if (!accessToken) {
    ...
    if (result.error && !result.success) {
      return sendError(res, 400, result.error.message || 'Failed to delete ad set')
    }
    ...
    sendError(res, 500, 'Failed to delete ad set', err)
  }
}

// ==================== ADS ====================

export async function getAds(pool: Pool, req: Request, res: Response) {
  try {
    await ensureTables(pool)
    const { adset_id, campaign_id } = req.query as any
    ...
    sendError(res, 500, 'Failed to get ads', err)
  }
}

export async function createAdCreative(pool: Pool, req: Request, res: Response) {
  try {
    await ensureTables(pool)
    const accessToken = await getMetaAccessToken(pool)
    const adAccountId = await getAdAccountId(pool)
    ...
    const {
      name,
      object_story_spec, // For single image/video ads
      link_url,
      image_url,
      video_id,
      call_to_action_type,
    ...
    if (!name || !link_url) {
      return sendError(res, 400, 'Creative name and link URL are required')
    }
    ...
    if (result.error) {
      return sendError(res, 400, result.error.message || 'Failed to create ad creative', result.error)
    }

    sendSuccess(res, { creative_id: result.id, ...result }, 201)
  } catch (err: any) {
    console.error('Creative creation error:', err)
    sendError(res, 500, 'Failed to create ad creative', err)
  }
}

async function getPageId(pool: Pool): Promise<string | null> {
  try {
    const { rows } = await pool.query(
      'SELECT page_id FROM facebook_config ORDER BY created_at DESC LIMIT 1'
    )
...
async function createAdCreativeHelper(
  pool: Pool,
  accessToken: string,
  adAccountId: string,
  creativeData: any
...
export async function createAd(pool: Pool, req: Request, res: Response) {
  try {
    await ensureTables(pool)
    const accessToken = await getMetaAccessToken(pool)
    const adAccountId = await getAdAccountId(pool)
    ...
    const {
      ...
      name,
      status = 'PAUSED',
      creative_id, // Use creative_id instead of full creative object
      creative, // Or use full creative object
    } = req.body

    if (!adset_id || !name) {
      return sendError(res, 400, 'Ad set ID and name are required')
    }

    // Get campaign_id from adset
    const { rows: adSetRows } = await pool.query(
      'SELECT campaign_id FROM meta_ad_sets WHERE adset_id = $1',
      [adset_id]
    )
    ...
    if (!finalCreativeId && creative) {
      ...
      try {
        ...
        const creativeResult = await createAdCreativeHelper(
          pool,
          accessToken,
          adAccountId,
          creativeData
        ...
        return sendError(res, 400, err.message || 'Failed to create creative')
      }
    }

    if (!finalCreativeId) {
      return sendError(res, 400, 'Creative ID or creative object is required')
    }
    ...
    if (result.error) {
      return sendError(res, 400, result.error.message || 'Failed to create ad', result.error)
    }

    const adId = result.id

    // Fetch full ad details
    const adDetails = await callMetaAPI(
      adId,
      'GET',
      { fields: 'id,name,status,creative,preview_url,created_time,updated_time' },
      accessToken
    )
    ...
    sendSuccess(res, { ad_id: adId, ...adDetails }, 201)
  } catch (err: any) {
    console.error('Ad creation error:', err)
    sendError(res, 500, 'Failed to create ad', err)
  }
}

export async function updateAd(pool: Pool, req: Request, res: Response) {
  try {
    const { id } = req.params as any
    const accessToken = await getMetaAccessToken(pool)
    if (!accessToken) {
    ...
    if (result.error) {
      return sendError(res, 400, result.error.message || 'Failed to update ad')
    }
    ...
    sendError(res, 500, 'Failed to update ad', err)
  }
}

export async function deleteAd(pool: Pool, req: Request, res: Response) {
  try {
    const { id } = req.params as any
    const accessToken = await getMetaAccessToken(pool)
    if (!accessToken) {
    ...
    if (result.error && !result.success) {
      return sendError(res, 400, result.error.message || 'Failed to delete ad')
    }
    ...
    sendError(res, 500, 'Failed to delete ad', err)
  }
}
...
export async function getInsights(pool: Pool, req: Request, res: Response) {
  try {
    ...
    sendError(res, 500, 'Failed to get insights', err)
  }
}

export async function syncInsights(pool: Pool, req: Request, res: Response) {
  try {
    await ensureTables(pool)
    const accessToken = await getMetaAccessToken(pool)
    if (!accessToken) {
...
export async function trackPixelEvent(pool: Pool, req: Request, res: Response) {
  try {
    await ensureTables(pool)
    const {
      event_name,
    ...
    sendError(res, 500, 'Failed to track pixel event', err)
  }
}

export async function getPixelEvents(pool: Pool, req: Request, res: Response) {
  try {
    await ensureTables(pool)
    const { event_name, limit = 100 } = req.query as any
    ...
    sendError(res, 500, 'Failed to get pixel events', err)
  }
}

// ==================== AUDIENCES ====================

export async function syncCustomAudience(pool: Pool, req: Request, res: Response) {
  try {
    const accessToken = await getMetaAccessToken(pool)
    const adAccountId = await getAdAccountId(pool)
    ...
    if (!name) {
      return sendError(res, 400, 'Audience name is required')
    }
    ...
    if (is_lookalike && source_audience_id) {
      ...
      if (customer_list && Array.isArray(customer_list) && customer_list.length > 0) {
        const payload = {
          ...
          data: customer_list.map((customer: any) => [
            customer.email || '',
            customer.phone || '',
            customer.first_name || '',
            customer.last_name || '',
    ...
    if (result.error) {
      return sendError(res, 400, result.error.message || 'Failed to sync audience', result.error)
    }

    sendSuccess(res, { audience_id: result.id, ...result })
  } catch (err: any) {
    console.error('Audience sync error:', err)
    sendError(res, 500, 'Failed to sync custom audience', err)
  }
}

export async function getAudiences(pool: Pool, req: Request, res: Response) {
  try {
    const accessToken = await getMetaAccessToken(pool)
    const adAccountId = await getAdAccountId(pool)
    ...
    const result = await callMetaAPI(
      `${adAccountId}/customaudiences`,
      'GET',
      { fields: 'id,name,subtype,approximate_count,description,created_time' },
      accessToken
    )

    if (result.error) {
      return sendError(res, 400, result.error.message || 'Failed to get audiences')
    }

    sendSuccess(res, Array.isArray(result.data) ? result.data : [result])
  } catch (err: any) {
    sendError(res, 500, 'Failed to get audiences', err)
  }
}
