...
export function registerCommunicationsRoutes(app: express.Express, pool: Pool, io: any) {
  ...
  app.get('/api/forms/submissions', async (_req, res) => {
    try {
      ...
      `)
      sendSuccess(res, { submissions: rows })
    } catch (err) {
  ...
  app.get('/api/forms/:id', async (req, res) => {
    try {
      ...
      sendError(res, 500, 'Failed to fetch form', err)
    }
  })
  ...
  app.post('/api/forms', async (req, res) => {
    try {
      ...
      if (!name) return sendError(res, 400, 'Form name is required')
      ...
      sendError(res, 500, 'Failed to create form', err)
    }
  })
  ...
  app.put('/api/forms/:id', async (req, res) => {
    try {
      ...
      sendError(res, 500, 'Failed to update form', err)
    }
  })
  ...
  app.delete('/api/forms/:id', async (req, res) => {
    try {
      ...
      sendError(res, 500, 'Failed to delete form', err)
    }
  })
  ...
  app.post('/api/forms/:id/submit', async (req, res) => {
    try {
      ...
      if (!data) return sendError(res, 400, 'Form data is required')

      // Check if form exists
      const formCheck = await pool.query('SELECT * FROM forms WHERE id = $1', [id])
      if (formCheck.rows.length === 0) return sendError(res, 404, 'Form not found')
      ...
      sendError(res, 500, 'Failed to submit form', err)
    }
  })
